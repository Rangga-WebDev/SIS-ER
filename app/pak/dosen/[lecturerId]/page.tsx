...
export default async function PakDosenDetailPage({
  params,
}: {
  ...
  // Hanya dosen yang DUPAK-nya sedang ditugaskan ke penilai ini yang boleh dibuka.
  const assignment = await getActivePakAssignmentForLecturer(
    user.id,
    lecturerId,
  );
  ...
  if (!lecturer) redirect("/pak/tugas");
  ...
  await logAudit({
    ...
    actorRole: user.role,
    action: "PAK_OPEN_LECTURER_DETAIL",
  ...
  return (
    <AppShell
      ...
      subtitle="Detail profil dan dokumen dosen yang ditugaskan kepada Anda (hanya baca)."
    >
      <div className="space-y-6">
        <Link
          href={`/pak/tugas/${assignment.id}`}
  ...
  return (
    <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-6 text-center">
      ...
      <p className="mx-auto mt-2 max-w-xl text-sm leading-6 text-slate-500">
        {isMetadataOnly
          ? "Dosen belum mengisi data untuk syarat ini."
          : "Dosen belum mengirim data untuk syarat ini."}
      </p>
    </div>
  );
}
